...
[DebuggerDisplay("{Metrics.Count} metrics, {cells.Count} cells")]
public class PhdRow(PhdEntry entry)
{
    ...
    public List<PhdMetric> Metrics { get; } = [];
    ...
    public Sample ToSample() => Metrics.Select(metric => metric.Value).ToSample(Metrics.First().Unit);
}